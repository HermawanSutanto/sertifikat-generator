...
const TutorialModal = ({ isOpen, onClose, isDark }) => {
  ...
  const steps = [
    ...
    },
    {
      ...
      tab: "Elemen",
      desc: "Tersedia elemen teks statis, teks variabel mandiri, serta integrasi aset resmi (Logo & Tanda Tangan) dari profil organisasi Anda yang dapat dipasang ke kanvas hanya dalam satu kali klik.",
    },
    {
...
export default function CetakLokal() {
  ...
  // State Aset Branding dari Profil Lembaga / Ruang Kerja
  ...
  useEffect(() => {
    ...
    const loadEventFromCloud = async () => {
      try {
        ...
        // 1. Terapkan konfigurasi layout tersimpan
        ...
        // 2. Terapkan filename pattern
        if (data.filenamePattern) {
          setFilenamePattern(data.filenamePattern);
        }

        // 3. Muat template PDF dari Supabase Storage jika ada
        ...
        // 4. Muat custom font dari Supabase jika ada
        ...
        // 5. Muat data peserta dari subkoleksi Firestore jika ada
        try {
  ...
  const handleSaveToCloud = async () => {
    ...
    try {
      ...
      // 1. Upload Template PDF jika ada file aktif
      if (templateFile && originalTemplateRawFile) {
        setCloudSaveStatus("Mengunggah template PDF ke Supabase Storage...");
      ...
      // 2. Upload Custom Font jika dipilih
      if (selectedFontBytes && selectedLocalFontFamily) {
        setCloudSaveStatus("Mengunggah font kustom ke Supabase Storage...");
      ...
      // 3. Upload elemen gambar ke Supabase Storage
      const imageConfigs = configs.filter((c) => c.enabled && c.type === "image" && c.data_url);
      if (imageConfigs.length > 0) {
        setCloudSaveStatus("Mengunggah elemen gambar ke Supabase Storage...");
      ...
      // 4. Simpan Data Peserta CSV ke Subkoleksi Firestore hanya bila berkas CSV baru diunggah
      if (hasNewCsvUpload && csvRows && csvRows.length > 0) {
        setCloudSaveStatus(`Menyimpan ${csvRows.length} data peserta ke Firestore...`);
        setHasNewCsvUpload(false);
        const pesertaColRef = collection(db, `events/${eventId}/peserta`);
        const existingSnap = await getDocs(pesertaColRef);

        // Hapus data peserta lama jika ada (chunked per 400 operasi)
        ...
        // Tulis data peserta baru (chunked per 400 operasi)
      ...
      // 5. Simpan Metadata, Configs, dan Total Peserta ke Dokumen Firestore
      setCloudSaveStatus("Menyimpan tata letak ke Firestore...");
      const docRef = doc(db, "events", eventId);
      ...
      setNotification({
        show: true,
        message: "Perubahan & data peserta berhasil disinkronkan ke Cloud!",
        type: "success",
      });
    } catch (err) {
  ...
  useEffect(() => {
    ...
    // Hanya tampilkan banner restore jika TIDAK dalam mode eventId
    if (eventId) return;
  ...
  const handleInsertProfileAsset = async (assetType) => {
    ...
    try {
      ...
      // Konversi berkas ke DataURL agar aman dirender di kanvas dan PDF embedding
      ...
      img.onload = () => {
        ...
        const newConfig = {
          ...
          mime_type: mimeType,
          // Letakkan logo di kiri atas secara default, dan TTD di kanan bawah
  ...
  return (
    ...
    >
      ...
      {/* BANNER RESTORE SESI TERSIMPAN (KHUSUS MODE MANDIRI) */}
      {isRestoreBannerOpen && (
        <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4">
          <div
            className={`border-2 rounded-[4px] max-w-md w-full p-6 space-y-4 shadow-2xl ${
      ...
      >
        <div className="flex items-center gap-4">
          ...
          {eventId && (
            <>
              <span className={isDark ? "text-[#444444]" : "text-[#CCCCCC]"}>/</span>
              <span className="text-xs font-mono font-bold text-[#0000EE] bg-blue-50 px-2 py-0.5 rounded">
        ...
        <div className="flex items-center gap-3">
          {/* Tombol Simpan ke Cloud Khusus Mode Event */}
          {eventId && (
            <button
              type="button"
              onClick={handleSaveToCloud}
              disabled={isCloudSaving}
              className="px-4 py-2 text-xs font-mono uppercase font-bold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 rounded-[4px] border border-emerald-700 flex items-center gap-1.5 shadow-sm transition-colors"
            >
              {isCloudSaving ? <Spinner /> : <span>💾</span>}
          ...
          <Link
            ...
            title={eventId ? "Kembali ke Detail Event" : "Kembali ke Dashboard"}
          >
          ...
          <div className="p-5 space-y-6">
            ...
            {activeTab === "elements" && (
              <div className="space-y-5">
                ...
                {/* SEKSI ASET RESMI DARI PROFIL LEMBAGA */}
                <div
                  className={`border rounded-[4px] p-3 space-y-2.5 ${
                    isDark ? "bg-[#111111] border-[#333333]" : "bg-[#F7F6F3] border-[#CCCCCC]"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <label className={`block text-[10px] font-mono uppercase font-bold ${isDark ? "text-[#AAAAAA]" : "text-[#555555]"}`}>
                      Aset Resmi dari Profil
                    </label>
                    {orgBranding.namaOrganisasi && (
                      <span className="text-[9px] font-mono text-[#0000EE] font-bold truncate max-w-[130px]">
                        {orgBranding.namaOrganisasi}
                      </span>
                    )}
                  </div>
                  ...
                  {isLoadingOrgBranding ? (
                    <div className="py-2 flex items-center justify-center gap-1.5 text-xs font-mono text-[#777777]">
                      <Spinner />
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className={`p-2 rounded-[4px] border text-left flex flex-col justify-between gap-1.5 transition-all ${
                          !orgBranding.logoUrl
                            ...
                            : isDark
                            ? "bg-[#181818] border-[#444444] hover:border-[#0000EE]"
                            : "bg-white border-[#CCCCCC] hover:border-[#0000EE] hover:shadow-xs"
                        }`}
                        title={orgBranding.logoUrl ? "Klik untuk memasang logo ke kanvas" : "Logo belum diunggah di profil"}
                      >
                        <div className="flex items-center justify-between w-full">
                          <span className="text-[10px] font-mono font-bold uppercase">
                            🖼️ {orgBranding.tipeOrganisasi === "personal" ? "Logo Jenama" : "Logo Lembaga"}
                          </span>
                          {orgBranding.logoUrl && (
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                          )}
                        </div>
                        <div className="h-10 w-full flex items-center justify-center bg-transparent overflow-hidden">
                          {orgBranding.logoUrl ? (
                            <img
                            ...
                            <span className="text-[9px] font-mono text-[#888888]">Kosong</span>
                          )}
                        </div>
                        <span className="text-[9px] font-mono font-bold text-[#0000EE] text-center w-full">
                          + Pasang Logo
                        </span>
                      </button>
                      ...
                      <button
                        ...
                        title={
                          orgBranding.capStempelUrl
                            ? "Klik untuk memasang cap/TTD ke kanvas"
                            : "Cap/TTD belum diunggah di profil"
                        }
                      >
                        <div className="flex items-center justify-between w-full">
                          <span className="text-[10px] font-mono font-bold uppercase">
                            🖋️ {orgBranding.tipeOrganisasi === "personal" ? "Tanda Tangan" : "Cap Stempel"}
                          </span>
                          {orgBranding.capStempelUrl && (
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                          )}
                    ...
                    <div className="text-center py-2 space-y-1">
                      <p className={`text-[10px] font-mono ${isDark ? "text-[#888888]" : "text-[#777777]"}`}>
                        Belum ada logo atau tanda tangan diatur.
                      </p>
                      <Link
                        href="/dashboard/profile"
                        target="_blank"
                        className="text-[10px] font-mono text-[#0000EE] hover:underline font-bold inline-block"
                      >
                        Unggah di Profil & Lembaga ↗
                      </Link>
                    </div>
                  )}
                </div>
              ...
              <div className="space-y-5">
                ...
                >
                  ...
                  <div className="pt-1 border-t border-dashed border-[#444444]/40">
                    ...
                    >
                      <option value={250}>250 Berkas (Sangat Aman / Hemat RAM)</option>
                    ...
                    <p className={`text-[9px] font-mono mt-1 ${isDark ? "text-[#888888]" : "text-[#666666]"}`}>
                      Jika 1 kategori melebihi batas ini, arsip otomatis dipecah menjadi beberapa bagian (part 1, part 2, dst).
                    </p>
                  </div>
                </div>
